...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def forward(self, x, mask): #x is of shape (B, S, D)
        """
        ...
        * split D(dim) into (H(n_heads), W(width of head)) ; D = H * W
        ...
        q, k, v = self.proj_q(x), self.proj_k(x), self.proj_v(x) #note: the projection is done before splitting for the attn heads
        q, k, v = (split_last(x, (self.n_heads, -1)).transpose(1, 2) #split_last changes the shape of the tensor so that each head is a separate dimension
                   for x in [q, k, v])
        
        # (B, H, S, W) @ (B, H, W, S) -> (B, H, S, S) -softmax-> (B, H, S, S)
        scores = q @ k.transpose(-2, -1) / np.sqrt(k.size(-1)) # k.size(-1) will be the number of instances per attention head. So np.sqrt(k.size(-1)) = 8 from the configuration.
        if mask is not None:
